...
export default function BeginningBalanceEdit({ value, year, landscape }: Props) {
  ...
  if (editing) {
    return (
      <div className="flex items-center gap-2">
        <span className="text-xs text-slate-500">Beginning Balance {year}</span>
  ...
  return (
    <div className="flex items-center gap-2 group">
      <span className="text-xs text-slate-500">Beginning Balance {year}</span>
      <span className="text-sm font-mono font-semibold text-slate-700">{fmt(value)}</span>
      <button
        onClick={startEdit}
