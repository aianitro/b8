...
| P1-10-zod-contracts | 10 ([ITEM.md](tasks/P1-10-zod-contracts/ITEM.md)) | **G0 ✅** (3 spec cycles) **G1 ✅** (2 cycles) 2026-09-12 — [SPEC.md](tasks/P1-10-zod-contracts/SPEC.md) frozen, [CONTRACT.md](tasks/P1-10-zod-contracts/CONTRACT.md), [GATES.md](tasks/P1-10-zod-contracts/GATES.md). Scoped to the zod schemas only; the workspaces conversion and the `/api/v1/*` migration are non-goals with their own scope commands and become successor tasks. **BLOCKED before G2 on an owner action**: the scope guard treats `shared/contracts/**/*.test.ts` as contract surface, so with the lease correctly closed the implementer cannot write the tests the spec requires and G2 is unreachable. A validated patch is prepared; the harness refuses to let an agent modify the guard. Neither the frozen spec nor the passed contract diff needs to change | — | `shared/contracts/**` (new), `shared/types.ts` (comment-only), `package.json` |
