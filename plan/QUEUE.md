...
| P1-10-zod-contracts | 10 ([ITEM.md](tasks/P1-10-zod-contracts/ITEM.md)) | **G0 ✅** 2026-09-11 (2 spec cycles; [SPEC.md](tasks/P1-10-zod-contracts/SPEC.md) frozen, [GATES.md](tasks/P1-10-zod-contracts/GATES.md)). Scoped to the zod schemas only — the workspaces conversion and the `/api/v1/*` migration are non-goals with their own scope commands, and become successor tasks. **Next: G1**, not skipped — `shared/contracts/**` is a new contract surface, so the guardian opens a lease | — | `shared/contracts/**` (new), `shared/types.ts`, `package.json` |
