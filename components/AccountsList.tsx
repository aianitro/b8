...
export default function AccountsList({ operational, capital, txnCounts }: Props) {
  ...
  // those changes (and any landscape move in/out of a group) show up here too.
  useEffect(() => { setOperationalOrder(operational); }, [operational]);
  useEffect(() => { setCapitalOrder(capital); }, [capital]);
  ...
  function Group({ title, items, section }: { title: string; items: Account[]; section: Section }) {
  ...
  return (
    <>
      <Group title="Operational" items={operationalOrder} section="operational" />
      <Group title="Capital" items={capitalOrder} section="capital" />
    </>
  );
}
