...
| T6 | a database of any kind, for the acceptance commands below | **NO** | n/a | n/a — every acceptance command is a pure `vitest` test, a `grep`/`node` check, or a compiler/build check | no fixture in this spec depends on dev-database state; the pg-parser measurements in the Goal section are evidence gathered once at spec time, not a runtime dependency of any command below |
...
| T8 | `typescript` resolves via `require("typescript")` from repo root — used by #40–#43's import-parsing check, not only by the `tsc` CLI | **yes** | already a resolvable devDependency | `node -e "console.log(require('typescript').version)"` | matches the version `npx tsc --version` reports |
...
>
> **Why #40–#43 parse rather than grep.** G1 measured that a text search on `shared/contracts` fails in both directions at once: it flags a comment merely *describing* compliance (`shared/contracts/envelope.ts` documented "no next/server, no pg, no @/lib/db," and a substring search matched its own sentence), and it misses a real violation written with a different quote style (`import db from "pg";` contains no literal substring `from 'pg'`). Both commands below instead parse each file with the `typescript` package — already a resolvable devDependency (T8), already the tool `npx tsc --noEmit` (#1) trusts to resolve this repo's modules — and walk its AST for `ImportDeclaration`/`ExportDeclaration` module specifiers, `require(...)` calls, and dynamic `import(...)` calls, checking only the decoded string content of those specific node kinds. A comment or an unrelated string literal produces none of those nodes and cannot match, whatever it says; a real import produces exactly one, whatever quote character or call form it uses. Before `shared/contracts/` exists, the script checks `fs.existsSync` first and reports `0` directly, rather than depending on a shell tool's exit behavior toward a missing path.
>
...
| 40 | `node -e 'const ts=require("typescript");const fs=require("fs");const path=require("path");const dir="shared/contracts";const forbidden="next/server";function walk(d){let out=[];for(const e of fs.readdirSync(d,{withFileTypes:true})){const p=path.join(d,e.name);if(e.isDirectory())out=out.concat(walk(p));else if(/\.tsx?$/.test(e.name))out.push(p);}return out;}let hits=0;if(fs.existsSync(dir)){for(const file of walk(dir)){const src=ts.createSourceFile(file,fs.readFileSync(file,"utf8"),ts.ScriptTarget.Latest);(function visit(node){let spec=null;if((ts.isImportDeclaration(node)\|\|ts.isExportDeclaration(node))&&node.moduleSpecifier){spec=node.moduleSpecifier.text;}else if(ts.isCallExpression(node)&&ts.isIdentifier(node.expression)&&node.expression.text==="require"&&node.arguments[0]&&ts.isStringLiteral(node.arguments[0])){spec=node.arguments[0].text;}else if(ts.isCallExpression(node)&&node.expression.kind===ts.SyntaxKind.ImportKeyword&&node.arguments[0]&&ts.isStringLiteral(node.arguments[0])){spec=node.arguments[0].text;}if(spec&&spec.includes(forbidden))hits++;ts.forEachChild(node,visit);})(src);}}console.log(hits);'` | `0` — no real import of `next/server` anywhere in `shared/contracts` (comments mentioning it do not count) |
| 41 | *(the same script with `forbidden="pg"` — see verbatim block)* | `0` — no real import of `pg`, whichever quote style or `require`/`import`/dynamic-`import` form is used |
| 42 | *(the same script with `forbidden="lib/db"` — see verbatim block)* | `0` — no real import of `@/lib/db` |
| 43 | *(the same script with `forbidden="categoryControl"` — see verbatim block)* | `0` — no real import of `lib/categoryControl` |
...
# 40 — no real import of next/server, anywhere in shared/contracts, however written.  Expect: 0
node -e 'const ts=require("typescript");const fs=require("fs");const path=require("path");const dir="shared/contracts";const forbidden="next/server";function walk(d){let out=[];for(const e of fs.readdirSync(d,{withFileTypes:true})){const p=path.join(d,e.name);if(e.isDirectory())out=out.concat(walk(p));else if(/\.tsx?$/.test(e.name))out.push(p);}return out;}let hits=0;if(fs.existsSync(dir)){for(const file of walk(dir)){const src=ts.createSourceFile(file,fs.readFileSync(file,"utf8"),ts.ScriptTarget.Latest);(function visit(node){let spec=null;if((ts.isImportDeclaration(node)||ts.isExportDeclaration(node))&&node.moduleSpecifier){spec=node.moduleSpecifier.text;}else if(ts.isCallExpression(node)&&ts.isIdentifier(node.expression)&&node.expression.text==="require"&&node.arguments[0]&&ts.isStringLiteral(node.arguments[0])){spec=node.arguments[0].text;}else if(ts.isCallExpression(node)&&node.expression.kind===ts.SyntaxKind.ImportKeyword&&node.arguments[0]&&ts.isStringLiteral(node.arguments[0])){spec=node.arguments[0].text;}if(spec&&spec.includes(forbidden))hits++;ts.forEachChild(node,visit);})(src);}}console.log(hits);'

# 41 — no real import of pg.  Expect: 0
node -e 'const ts=require("typescript");const fs=require("fs");const path=require("path");const dir="shared/contracts";const forbidden="pg";function walk(d){let out=[];for(const e of fs.readdirSync(d,{withFileTypes:true})){const p=path.join(d,e.name);if(e.isDirectory())out=out.concat(walk(p));else if(/\.tsx?$/.test(e.name))out.push(p);}return out;}let hits=0;if(fs.existsSync(dir)){for(const file of walk(dir)){const src=ts.createSourceFile(file,fs.readFileSync(file,"utf8"),ts.ScriptTarget.Latest);(function visit(node){let spec=null;if((ts.isImportDeclaration(node)||ts.isExportDeclaration(node))&&node.moduleSpecifier){spec=node.moduleSpecifier.text;}else if(ts.isCallExpression(node)&&ts.isIdentifier(node.expression)&&node.expression.text==="require"&&node.arguments[0]&&ts.isStringLiteral(node.arguments[0])){spec=node.arguments[0].text;}else if(ts.isCallExpression(node)&&node.expression.kind===ts.SyntaxKind.ImportKeyword&&node.arguments[0]&&ts.isStringLiteral(node.arguments[0])){spec=node.arguments[0].text;}if(spec&&spec.includes(forbidden))hits++;ts.forEachChild(node,visit);})(src);}}console.log(hits);'

# 42 — no real import of @/lib/db.  Expect: 0
node -e 'const ts=require("typescript");const fs=require("fs");const path=require("path");const dir="shared/contracts";const forbidden="lib/db";function walk(d){let out=[];for(const e of fs.readdirSync(d,{withFileTypes:true})){const p=path.join(d,e.name);if(e.isDirectory())out=out.concat(walk(p));else if(/\.tsx?$/.test(e.name))out.push(p);}return out;}let hits=0;if(fs.existsSync(dir)){for(const file of walk(dir)){const src=ts.createSourceFile(file,fs.readFileSync(file,"utf8"),ts.ScriptTarget.Latest);(function visit(node){let spec=null;if((ts.isImportDeclaration(node)||ts.isExportDeclaration(node))&&node.moduleSpecifier){spec=node.moduleSpecifier.text;}else if(ts.isCallExpression(node)&&ts.isIdentifier(node.expression)&&node.expression.text==="require"&&node.arguments[0]&&ts.isStringLiteral(node.arguments[0])){spec=node.arguments[0].text;}else if(ts.isCallExpression(node)&&node.expression.kind===ts.SyntaxKind.ImportKeyword&&node.arguments[0]&&ts.isStringLiteral(node.arguments[0])){spec=node.arguments[0].text;}if(spec&&spec.includes(forbidden))hits++;ts.forEachChild(node,visit);})(src);}}console.log(hits);'

# 43 — no real import of lib/categoryControl.  Expect: 0
node -e 'const ts=require("typescript");const fs=require("fs");const path=require("path");const dir="shared/contracts";const forbidden="categoryControl";function walk(d){let out=[];for(const e of fs.readdirSync(d,{withFileTypes:true})){const p=path.join(d,e.name);if(e.isDirectory())out=out.concat(walk(p));else if(/\.tsx?$/.test(e.name))out.push(p);}return out;}let hits=0;if(fs.existsSync(dir)){for(const file of walk(dir)){const src=ts.createSourceFile(file,fs.readFileSync(file,"utf8"),ts.ScriptTarget.Latest);(function visit(node){let spec=null;if((ts.isImportDeclaration(node)||ts.isExportDeclaration(node))&&node.moduleSpecifier){spec=node.moduleSpecifier.text;}else if(ts.isCallExpression(node)&&ts.isIdentifier(node.expression)&&node.expression.text==="require"&&node.arguments[0]&&ts.isStringLiteral(node.arguments[0])){spec=node.arguments[0].text;}else if(ts.isCallExpression(node)&&node.expression.kind===ts.SyntaxKind.ImportKeyword&&node.arguments[0]&&ts.isStringLiteral(node.arguments[0])){spec=node.arguments[0].text;}if(spec&&spec.includes(forbidden))hits++;ts.forEachChild(node,visit);})(src);}}console.log(hits);'
...
| 13 | The contract surface has no *real import* of the code it must not depend on — a comment naming the forbidden module does not violate the rule, and no quote style or call form (`import`, `require`, dynamic `import()`) may evade it | a genuine `import`/`require`/dynamic-`import` of `next/server`, `pg`, `@/lib/db`, or `lib/categoryControl` inside `shared/contracts`, in any of those forms | acceptance #40–#43 |
...
- A layering/derivation-source check implemented as a text search over `shared/contracts` — flags a comment that documents the very rule it enforces, and misses a real import written with a different quote character or as a bare `require`. This is not hypothetical: it is the exact defect G1 measured in this spec's own first draft of acceptance #40–#43, corrected above.
