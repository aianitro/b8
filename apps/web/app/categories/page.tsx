...
import type { BudgetCategory, Account } from '@b8/contracts/types';
import CategoryManager from '@/components/CategoryManager';

async function getCategories(): Promise<BudgetCategory[]> {
  const result = await db.query<BudgetCategory>(
    'SELECT id, name, annual_budget, landscape, exclude_from_budget, is_income, control_mode, dedicated_account_id, monthly_amounts, created_at FROM budget_categories ORDER BY is_income, exclude_from_budget, landscape, name'
  );
  return result.rows;
}
