...
// explicit due-months schedule — i.e. spend landed outside its expected window. Distinct from
// "no budget configured at all" (monthlyBudget === 0 with no schedule), which stays neutral,
// same as it always has.
function monthPct(spent: number, monthlyBudget: number, offCycle: boolean): number {
...
function expenseCellStyle(spent: number, monthlyBudget: number, isFuture: boolean, offCycle: boolean): string {
...
function expenseCellText(spent: number, monthlyBudget: number, isFuture: boolean, offCycle: boolean): string {
...
function incomeCellStyle(received: number, monthlyTarget: number, isFuture: boolean, offCycle: boolean): string {
  if (isFuture) return 'bg-slate-50';
...
function incomeCellText(received: number, isFuture: boolean): string {
...
export default function BudgetMonthlyGridClient({ rows, currentMonth, beginningBalance, uncategorizedIncome, uncategorizedExpense }: Props) {
  ...
  const incomeMonthTotals  = Array.from({ length: 12 }, (_, i) => incomeOrder.reduce((s, r) => s + r.months[i], 0) + uncategorizedIncome[i]);
  const expenseMonthTotals = Array.from({ length: 12 }, (_, i) => expenseOrder.reduce((s, r) => s + r.months[i], 0) + uncategorizedExpense[i]);
  ...
  function Row({ row, section }: { row: GridRow; section: Section }) {
    ...
    return (
      ...
      >
        {/* Category */}
        <td className="sticky left-0 bg-white group-hover:bg-slate-50/30 px-3 py-2 border-r border-slate-100 z-10 min-w-[180px]">
          <div className="flex items-center gap-1.5">
            <GripVertical size={13} className="text-slate-300 group-hover:text-slate-400 shrink-0 transition-colors" />
            ...
            >
              <div className="text-sm font-medium text-slate-800 whitespace-nowrap hover:text-blue-600 hover:underline">{row.category}</div>
        ...
        {row.months.map((amount, i) => {
          ...
          return (
            <td
              key={i}
              title={isFuture ? MONTHS[i] : `${MONTHS[i]}: ${prefix}${fmtFull(Math.abs(amount))} (${tooltipBudget})`}
              className={`p-0 text-right text-xs min-w-[80px] border-r border-slate-50 transition-colors ${bg} ${text} ${i === currentMonth ? 'ring-1 ring-inset ring-blue-200' : ''} ${!isFuture ? 'hover:brightness-95' : ''}`}
            >
              {drillHref ? (
                <a href={drillHref} className="block px-3 py-2 w-full h-full">{inner}</a>
              ) : (
        ...
        {/* YTD */}
        <td className="sticky right-0 bg-white group-hover:bg-slate-50/30 px-4 py-2 text-right border-l border-slate-100 z-10 min-w-[110px]">
          ...
          <div className={`text-[10px] mt-0.5 ${ytdPct > 100 && !row.is_income ? 'text-red-500' : 'text-slate-400'}`}>
  ...
  function SectionHeader({ title }: { title: string }) {
    return (
      <tr>
        <td
          colSpan={15}
          className="sticky left-0 bg-slate-50 px-5 py-2 text-[10px] font-semibold uppercase tracking-widest text-slate-400 border-b border-slate-100"
        >
          {title}
        </td>
      </tr>
  ...
  function TotalsRow({ label, monthTotals, ytd, accent, isIncome }: { label: string; monthTotals: number[]; ytd: number; accent: string; isIncome?: boolean }) {
    return (
      <tr className="bg-slate-50 border-t border-slate-200">
        <td className={`sticky left-0 px-5 py-2 text-xs font-bold uppercase tracking-wider text-slate-500 border-r border-slate-200 z-10 ${accent}`}>
          {label}
        </td>
        {monthTotals.map((total, i) => {
          ...
          return (
            <td
              key={i}
              className={`px-3 py-2 text-right text-xs font-mono font-semibold border-r border-slate-100 ${
                isFuture ? 'text-slate-200' : negative ? 'text-red-600' : 'text-slate-700'
        ...
        <td className={`sticky right-0 px-4 py-2 text-right font-mono font-bold border-l border-slate-200 z-10 ${isIncome && ytd < 0 ? 'text-red-600' : 'text-slate-700'} ${accent}`}>
          {fmt(ytd)}
        </td>
      </tr>
    );
  ...
  function NetRow() {
    return (
      <tr className="border-t-2 border-slate-300">
        <td className="sticky left-0 bg-slate-900 px-5 py-2 text-xs font-bold uppercase tracking-wider text-white border-r border-slate-700 z-10">
          Net
        </td>
        {netMonthTotals.map((net, i) => {
          const isFuture = i > currentMonth;
          const color = isFuture ? 'text-slate-600' : net > 0 ? 'text-emerald-400' : net < 0 ? 'text-red-400' : 'text-slate-500';
          return (
            <td
              key={i}
              className={`px-3 py-2 text-right text-xs font-mono font-bold border-r border-slate-700 bg-slate-900 ${color} ${i === currentMonth ? 'bg-slate-800' : ''}`}
            >
              {!isFuture ? fmtSigned(net) : ''}
            </td>
          );
        })}
        <td className={`sticky right-0 bg-slate-900 px-4 py-2 text-right font-mono font-bold border-l border-slate-700 z-10 ${netYtd > 0 ? 'text-emerald-400' : netYtd < 0 ? 'text-red-400' : 'text-white'}`}>
          {fmtSigned(netYtd)}
        </td>
      </tr>
    );
  ...
  function UncategorizedRow({ months, isIncome }: { months: number[]; isIncome: boolean }) {
    ...
    return (
      <tr className="border-b border-amber-100 bg-amber-50/40 hover:bg-amber-50/70 transition-colors">
        <td className="sticky left-0 bg-amber-50/40 px-3 py-2 border-r border-amber-100 z-10 min-w-[180px]">
          <div className="flex items-center gap-1.5">
            <div className="w-4 shrink-0" />
            <div className="w-0.5 h-5 rounded-full shrink-0 bg-amber-400" />
            <div className="text-sm font-medium text-amber-700 whitespace-nowrap">Uncategorized</div>
          </div>
        </td>
        {months.map((amount, i) => {
          const isFuture = i > currentMonth;
          ...
          return (
            <td
              key={i}
              className={`p-0 text-right text-xs min-w-[80px] border-r border-amber-100 ${i === currentMonth ? 'bg-amber-50' : ''}`}
            >
              {href ? (
                <a href={href} className="block px-3 py-2 w-full h-full hover:bg-amber-100/50 transition-colors">{inner}</a>
              ) : (
                <div className="px-3 py-2">{inner}</div>
              )}
            </td>
        ...
        <td className="sticky right-0 bg-amber-50/40 px-4 py-2 text-right border-l border-amber-100 z-10 min-w-[110px]">
          <div className={`font-mono text-sm font-semibold ${isIncome ? 'text-emerald-700' : 'text-amber-700'}`}>
            {ytd > 0 ? fmt(ytd) : '—'}
          </div>
        </td>
      </tr>
  ...
  function BalanceRow({ label, balances, ytdBalance, accent }: { label: string; balances: number[]; ytdBalance: number; accent: string }) {
    return (
      <tr className={`border-t border-slate-200 ${accent}`}>
        <td className={`sticky left-0 px-5 py-2 text-xs font-bold uppercase tracking-wider text-slate-600 border-r border-slate-200 z-10 ${accent}`}>
          {label}
        </td>
        {balances.map((bal, i) => {
          const isFuture = i > currentMonth;
          const color = isFuture ? 'text-slate-300' : bal >= 0 ? 'text-slate-700' : 'text-red-600';
          return (
            <td
              key={i}
              className={`px-3 py-2 text-right text-xs font-mono font-semibold border-r border-slate-100 ${color} ${i === currentMonth ? 'bg-blue-50/40' : ''}`}
            >
              {!isFuture ? fmt(bal) : ''}
            </td>
          );
        })}
        <td className={`sticky right-0 px-4 py-2 text-right font-mono font-semibold border-l border-slate-200 z-10 ${accent} ${ytdBalance >= 0 ? 'text-slate-700' : 'text-red-600'}`}>
          {fmt(ytdBalance)}
        </td>
      </tr>
    );
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        <table className="w-full text-sm border-collapse">
          <thead className="sticky top-0 z-20">
            <tr className="bg-slate-50 border-b border-slate-100">
              <th className="sticky left-0 bg-slate-50 px-5 py-2 text-left text-xs font-semibold uppercase tracking-wider text-slate-400 border-r border-slate-100 min-w-[180px] z-30">
                Category
              </th>
              {MONTHS.map((m, i) => (
                <th
                  key={m}
                  className={`px-3 py-2 text-right text-xs font-semibold uppercase tracking-wider min-w-[80px] border-r border-slate-50 ${
                    i === currentMonth
                ...
                >
                  {m}
                  {i === currentMonth && <div className="text-[9px] font-normal mt-0.5 text-blue-400">current</div>}
                </th>
              ))}
              <th className="sticky right-0 bg-slate-50 px-4 py-2 text-right text-xs font-semibold uppercase tracking-wider text-slate-400 border-l border-slate-100 min-w-[110px] z-30">
                YTD
              </th>
            </tr>
          </thead>
          <tbody>
            ...
            {incomeOrder.length > 0 && (
              <>
                ...
                {incomeOrder.map((row) => <Row key={row.id} row={row} section="income" />)}
                <UncategorizedRow months={uncategorizedIncome} isIncome={true} />
                <TotalsRow label="Total Income" monthTotals={incomeMonthTotals} ytd={incomeYtd} accent="bg-emerald-50" isIncome />
              </>
            )}
            ...
            {expenseOrder.length > 0 && (
              <>
                ...
                {expenseOrder.map((row) => <Row key={row.id} row={row} section="expense" />)}
                <UncategorizedRow months={uncategorizedExpense} isIncome={false} />
                <TotalsRow label="Total Expenses" monthTotals={expenseMonthTotals} ytd={expenseYtd} accent="bg-slate-100" />
              </>
            )}
      ...
      {/* Legend */}
      <div className="px-5 py-2 border-t border-slate-100 flex items-center gap-5 text-[11px] text-slate-400 flex-wrap">
