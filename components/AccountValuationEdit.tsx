...
interface Props {
  accountId: string;
  /** Latest recorded valuation, or null if this account has never been valued. */
  current: number | null;
  isLiability: boolean;
}
...
// Each save appends a new account_valuations row rather than editing the last one — the table
// is an observation history, so this is also how the quarterly re-valuation flow works.
export default function AccountValuationEdit({ accountId, current, isLiability }: Props) {
  ...
  async function save() {
    ...
    if (parsed < 0) {
      setError('Enter a positive amount — use "Valuation (liability)" for debts');
      return;
    }
  ...
  return (
    <div className="flex flex-col items-end">
      ...
      >
        {current !== null ? (
          <span className={`font-mono font-medium ${isLiability ? 'text-red-500' : 'text-slate-600'}`}>
            {isLiability ? `−${fmt(current)}` : fmt(current)}
          </span>
        ) : (
